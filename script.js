...
const I18N = {
  ua: {
    ...
    profileRole: 'Покупець',
    menuOrders: 'Мої замовлення',
    menuFavorites: 'Обране',
    menuAdmin: 'Адмін-панель',
    adminTitle: 'Адмін-панель',
    adminAddTitle: 'Додати товар',
    prodNamePh: 'Назва',
    prodPricePh: 'Ціна (₴)',
    prodDescPh: 'Опис',
    ...
    modalText: "Ми зв'яжемося з вами найближчим часом",
    modalBtn: 'Чудово',
    ...
    flowerTulip: 'Тюльпан',
    flowerDaisy: 'Ромашка',
    flowerSunflower: 'Соняшник',
    flowerLavender: 'Лаванда',
    favoritesTitle: 'Обране',
    favoritesEmptyTitle: 'Список порожній',
    favoritesEmptyText: 'Додайте товари до обраного',
    ordersTitle: 'Мої замовлення',
    ordersEmptyTitle: 'Замовлень немає',
    ordersEmptyText: 'Ви ще не робили замовлень',
  ...
  ru: {
    ...
    feat2Text: 'Поставка каждое утро',
    ...
    pdFeature3: 'Гарантия качества',
    addToCart: 'Добавить в корзину',
    addToFavorites: 'Добавить в избранное',
    removeFromFavorites: 'Удалить из избранного',
    cartTitle: 'Корзина',
    cartEmptyTitle: 'Корзина пуста',
    cartEmptyText: 'Добавьте цветы из каталога',
    cartTotalLabel: 'Итого:',
    checkoutBtn: 'Оформить заявку',
    ...
    flowerTulip: 'Тюльпан',
    flowerDaisy: 'Ромашка',
    flowerSunflower: 'Подсолнух',
    flowerLavender: 'Лаванда',
    favoritesTitle: 'Избранное',
    favoritesEmptyTitle: 'Список пуст',
    favoritesEmptyText: 'Добавьте товары в избранное',
    ordersTitle: 'Мои заказы',
    ordersEmptyTitle: 'Заказов нет',
    ordersEmptyText: 'Вы еще не делали заказов',
    orderNumber: 'Заказ №',
    orderDate: 'Дата:',
    orderTotal: 'Сумма:',
    orderStatus: 'Статус:'
    ...
    return `
      <div class="favorite-item" style="animation-delay: ${i * 0.05}s" data-id="${product.id}">
        ...
        <div class="favorite-item__actions">
          <button class="favorite-remove-btn" data-id="${product.id}" aria-label="Remove">
            <svg viewBox="0 0 24 24"><path d="M6 6 L18 18 M6 18 L18 6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
          </button>
        </div>
      </div>
    `;
  ...
  catalog.innerHTML = products.map((p, i) => `
    <div class="product-card" data-id="${p.id}" style="animation-delay: ${i * 0.05}s">
      <div class="product-image">
        ${getProductImage(p)}
        <button class="favorite-btn ${isFavorite(p.id) ? 'active' : ''}" data-id="${p.id}" aria-label="Favorite">
          <svg viewBox="0 0 24 24">
            <path class="heart-outline" d="M12 20 L10.5 18.5 C5 13.5, 3 11, 3 8 C3 5, 5 3, 8 3 C10 3, 11.5 4, 12 5.5 C12.5 4, 14 3, 16 3 C19 3, 21 5, 21 8 C21 11, 19 13.5, 13.5 18.5 Z"/>
            <path class="heart-filled" d="M12 20 L10.5 18.5 C5 13.5, 3 11, 3 8 C3 5, 5 3, 8 3 C10 3, 11.5 4, 12 5.5 C12.5 4, 14 3, 16 3 C19 3, 21 5, 21 8 C21 11, 19 13.5, 13.5 18.5 Z"/>
          </svg>
        </button>
      </div>
      ...
      <div class="product-bottom">
        ...
        <button class="add-btn" data-id="${p.id}" aria-label="Add">${ICON_PLUS}</button>
      </div>
    </div>
  `).join('');
...
function renderProductDetail(id) {
  ...
  const images = (product.images && product.images.length > 0)
    ? product.images
    : [FLOWER_SVGS[product.icon] || FLOWER_SVGS.rose];
  ...
  track.innerHTML = images.map((img, i) => `
    <div class="gallery__slide" data-index="${i}">
      ${isPhoto ? `<img src="${img}" alt="Фото ${i+1}" />` : img}
    </div>
  `).join('');
  ...
  setupGalleryScroll(track, dots, counter, images.length);
...
function setupGalleryScroll(track, dots, counter, total) {
  let scrollTimeout;
  ...
  list.innerHTML = cart.map((item, i) => {
    ...
    if (!p) {
      return `
        <div class="cart-item" style="animation-delay: ${i * 0.05}s; opacity: 0.6;">
          ...
          <button class="qty-btn" data-action="remove" data-id="${item.id}" style="background: var(--danger); color: #fff;">
            <svg viewBox="0 0 24 24"><path d="M6 6 L18 18 M6 18 L18 6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
          </button>
        </div>
      `;
    ...
    return `
      <div class="cart-item" style="animation-delay: ${i * 0.05}s">
        ...
        <div class="cart-item__info">
          <div class="cart-item__name">${escapeHtml(getLocalizedName(p))}</div>
          <div class="cart-item__price">${formatPrice(sum)}</div>
        </div>
        <div class="qty-controls">
          ...
          <button class="qty-btn" data-action="plus" data-id="${p.id}">
            <svg viewBox="0 0 24 24"><path d="M12 5 L12 19 M5 12 L19 12" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg>
          </button>
        </div>
      </div>
    `;
...
phoneInput.addEventListener('input', (e) => {
  ...
  if (!v.startsWith('380') && v.length > 0) {
    if (v.startsWith('3')) v = '380' + v.slice(1);
    else v = '380' + v;
  }
  v = v.slice(0, 12);
...
orderForm.addEventListener('submit', (e) => {
  ...
  if (name.length < 2) {
    phoneError.textContent = t('nameError');
    haptic('heavy');
    return;
  }
  if (phone.length !== 12) {
  ...
  const orderTotal = cart.reduce((s, i) => {
  ...
  if (tg) {
    try { tg.sendData(JSON.stringify(newOrder)); } catch(err){}
  }
